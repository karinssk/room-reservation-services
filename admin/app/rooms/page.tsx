      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {filteredRooms.map((room) => (
            <div
              key={room.id}
              className="rounded-2xl border border-slate-200 bg-white p-5 transition-all hover:border-blue-200 hover:shadow-md"
            >
              <div className="flex items-start gap-4">
                {room.coverImage && (
                  <img
                    src={resolveUploadUrl(room.coverImage)}
                    alt={getName(room.name)}
