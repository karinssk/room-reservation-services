...
export default function EditRoomPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // First fetch all rooms to find the one with matching ID
      ...
      const [roomRes, categoriesRes] = await Promise.all([
        fetch(`${API_URL}/rooms/${roomMatch.slug}`),
      ...
      const roomRecord = roomData.room;
      setRoom(roomRecord);
      setCategories(categoriesData.categories || []);

      // Populate form
      const name = typeof roomRecord.name === "string"
        ? { en: roomRecord.name, th: roomRecord.name }
        : roomRecord.name;
      const description = typeof roomRecord.description === "string"
        ? { en: roomRecord.description, th: roomRecord.description }
        : roomRecord.description;
      const shortDescription = typeof roomRecord.shortDescription === "string"
        ? { en: roomRecord.shortDescription, th: roomRecord.shortDescription }
        : roomRecord.shortDescription;

      setNameEn(name?.en || "");
      setNameTh(name?.th || "");
      setRoomSlug(roomRecord.slug || "");
      ...
      setShortDescriptionEn(shortDescription?.en || "");
      setShortDescriptionTh(shortDescription?.th || "");
      setPricePerNight(roomRecord.pricePerNight || 0);
      setPricePerMonth(roomRecord.pricePerMonth || 0);
      setTotalRooms(roomRecord.totalRooms || 1);
      setCoverImage(roomRecord.coverImage || "");
      setGallery(roomRecord.gallery || []);
      setBeddingOptions(
        roomRecord.beddingOptions && roomRecord.beddingOptions.length > 0
          ? roomRecord.beddingOptions
          : [{ type: "", description: "" }]
      );
      setFacilities(roomRecord.facilities || {
        bathroomFeatures: [],
        climateControl: [],
        entertainment: [],
        generalAmenities: [],
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500">Update room information</p>
      </div>
